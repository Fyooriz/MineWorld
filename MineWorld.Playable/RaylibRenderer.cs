...
/// <summary>Current GPU-backed renderer. Raylib owns the graphics context; world/gameplay remain engine-side.</summary>
internal sealed class RaylibRenderer : IRenderer
{
    ...
    public Camera3DState BeginFrame(Vector3 position, Vector3 target)
    {
        ...
        return new Camera3DState(position, target, Vector3.UnitY, 70f);
    }

    public void RenderWorld(VoxelWorld world) => world.Draw();

    public void DrawHud(PlayerController player, VoxelWorld world)
    {
        Raylib.EndMode3D();
        Raylib.DrawText("MINEWORLD P0 — GPU VOXEL SLICE", 20, 18, 24, Color.White);
